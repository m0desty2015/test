...
Abstract Class Model_Base {
	...
	public function update($field_u, $field_ures, $field_up, $field_res){
		$arrayAllFields = array_keys($this->fieldsTable());
		$arrayForSet = array();
		...
		foreach($arrayAllFields as $field){
			if(!empty($this->$field)){
				if(strtoupper($field) != 'ID'){
					$arrayForSet[$field] = $field . ' = "' . $this->$field . '"';
				}else{
					$whereID = $this->$field;
				}
			}
		...
		try {
			$db = $this->db;
			$db->beginTransaction();
			$stmt = $db->prepare("UPDATE $this->table SET $field_up=".$field_res."  WHERE `".$field_u."` = ".$field_ures);  
			//echo "UPDATE $this->table SET $strForSet WHERE `id` = $whereID";
			$result = $stmt->execute();
			$db->commit();
		}catch(PDOException $e){
			echo 'Error : '.$e->getMessage();
